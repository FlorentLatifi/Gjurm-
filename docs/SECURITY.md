...
| Vulnerable dependencies | Lockfiles (`uv.lock`, `package-lock.json`) installed frozen; `pip-audit` and `npm audit` in CI; **Trivy** fails the image build on fixable HIGH/CRITICAL; Dependabot weekly | `.github/workflows/ci.yml`, `.github/dependabot.yml` |
| Container escape / lateral movement | Non-root user (UID 10001), read-only root filesystem, `cap_drop: ALL`, `no-new-privileges`, tmpfs `/tmp`, memory limits. The database is on an internal network with no route out; the API has no internet egress; only Caddy publishes ports. | Dockerfiles, `deploy/docker-compose.prod.yml` |
