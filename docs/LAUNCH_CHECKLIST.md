...
| | Item | Evidence |
|---|---|---|
| ✅ | Pipeline: ingest → process → enrich → quality → alerts | 218 backend tests incl. 92 against PostgreSQL ([TESTING.md](TESTING.md)) |
...
| ✅ | Images build and pass Trivy | Caddy rebuilt from source (ADR-017); the api image passed and was pushed to GHCR |
...
1. ⬜ **Model quality spot-check.** With the key set, enrich ~50 real articles (`gjurme enrich --limit 50`). Read the topics, entities and summaries in the Articles page, check `admin quality` (ungrounded-entity and low-confidence rates), and check `admin costs` against the $0.004/article estimate. Optionally repeat with `LLM_MODEL=claude-haiku-4-5` and compare before deciding the default.
2. ⬜ First deploy: `./deploy.sh <sha>` ([DEPLOYMENT.md §6](DEPLOYMENT.md#6-first-deploy)).
3. ⬜ Verify: `/health/ready`, `/api/v1/status` = `ok` after the first run, HSTS and CSP headers, `/metrics` = 404, admin endpoint without a token = 401, `ufw status`.
